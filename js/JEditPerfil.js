(function () {
  ...
  function validateImage() {
    var photoInput = document.getElementById("avatar");
  ...
  function validateUsername() {
    ...
    if (username !== currentUsername) {
      ...
      fetch(
        `${API_BASE}php/sesion/check_username.php?username=` +
  ...
  function validateEmail() {
    ...
    var emailPattern =
      /^[^\s@]+@(outlook\.com|hotmail\.com|gmail\.com|yahoo\.com)$/i;

    if (email !== currentEmail) {
      if (!emailPattern.test(email)) {
        errorMessage.style.display = "block";
        errorMessage.textContent =
          "Por favor, ingresa un correo electrónico de un dominio válido (outlook, hotmail, gmail, yahoo).";
        return false;
      }

      fetch(
        `${API_BASE}php/sesion/check_email.php?email=` +
  ...
  function validateBirthdate() {
    ...
    if (birthdate.getTime() >= today.getTime()) {
      errorMessage.style.display = "block";
      errorMessage.textContent =
        "La fecha de nacimiento no puede ser la fecha actual ni una fecha futura.";
      return false;
    }
  ...
  function validatePassword() {
    ...
    var passwordPattern =
      /^(?=.*[a-zñÑ])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*()_+])[A-Za-zñÑ\d!@#$%^&*()_+]{8,}$/;

    if (!passwordPattern.test(password)) {
      errorMessage.style.display = "block";
      errorMessage.textContent =
        "La contraseña debe tener al menos 8 caracteres en total, incluyendo 1 mayuscula, 1 minuscula, 1 número y 1 símbolo especial";
      return false;
    }
